...
private:
    ...
    {
        const int *id = texto_a_id.buscar(vertice);
        if (id != nullptr)
            return *id;
        return -1;
    }
...
public:
    ...
    long long contarCaminosObl(const string &origen, const string &destino,
                               const std::vector<string> &nodosObligatorios) const
    {
